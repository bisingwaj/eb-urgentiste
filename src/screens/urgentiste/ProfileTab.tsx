      ...
      <View style={styles.topHeader}>
        ...
        <View style={styles.profileSummary}>
           ...
           <View style={styles.profileInfo}>
              <Text style={styles.agentName}>{profile?.first_name} {profile?.last_name}</Text>
              <Text style={styles.agentRank}>{profile?.role === 'secouriste' ? 'Médecin Urgentiste' : profile?.role}</Text>
              <View style={styles.idBadge}>
                 <Text style={styles.idBadgeText}>MATRICULE: {profile?.agent_login_id || profile?.matricule || 'XXX'}</Text>
              </View>
           </View>
        </View>
      </View>
        ...
        <View style={styles.menuList}>
           <TouchableOpacity style={styles.menuItem}>
              ...
              <View style={styles.menuText}>
                 ...
                 <Text style={styles.menuValue}>{profile?.phone || 'Non renseigné'}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  avatarCircle: { width: 70, height: 70, borderRadius: 30, backgroundColor: colors.secondary + '15', justifyContent: 'center', alignItems: 'center' },
  statusBadge: { position: 'absolute', bottom: 4, right: 4, width: 14, height: 14, borderRadius: 7, backgroundColor: colors.success, borderWidth: 2, borderColor: "#1A1A1A" },
  profileInfo: { flex: 1 },
  agentName: { fontSize: 22, fontWeight: '900', color: '#FFF', marginBottom: 4 },
  agentRank: { fontSize: 13, fontWeight: '700', color: colors.textMuted, marginBottom: 12 },
  ...
  statLabel: { fontSize: 11, fontWeight: '800', color: colors.textMuted, marginBottom: 4, letterSpacing: 0.5 },
  statValue: { fontSize: 15, fontWeight: '900', color: '#FFF' },

  sectionTitle: { color: "#FFF", fontSize: 13, fontWeight: "800", marginHorizontal: 30, marginTop: 30, marginBottom: 12, letterSpacing: 1.5, opacity: 0.4 },
  menuList: { backgroundColor: "#1A1A1A", marginHorizontal: 20, borderRadius: 32, overflow: 'hidden', borderWidth: 1, borderColor: "rgba(255,255,255,0.05)" },
  menuItem: { flexDirection: 'row', alignItems: 'center', padding: 20, borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.02)" },
  menuIcon: { width: 48, height: 48, borderRadius: 16, justifyContent: 'center', alignItems: 'center', marginRight: 20 },
  menuText: { flex: 1 },
  menuLabel: { fontSize: 16, fontWeight: '700', color: '#FFF' },
  menuValue: { fontSize: 14, color: colors.textMuted, marginTop: 2, fontWeight: '600' },
  ...
  logoutText: { color: colors.primary, fontSize: 15, fontWeight: '800', marginLeft: 12, letterSpacing: 0.5 },
});
