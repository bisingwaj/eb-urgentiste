...
export function HistoryTab({ navigation }: any) {
  ...
  const getOutcomeSpecs = (type: string) => {
    switch(type) {
      case 'completed': return { icon: "check-circle" as const, color: colors.success };
      case 'refused': return { icon: "cancel" as const, color: "#FF9800" };
      case 'critical': return { icon: "error" as const, color: colors.primary };
      default: return { icon: "check-circle" as const, color: colors.secondary };
    }
  };
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.topHeader}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerIconRow}>
            <TouchableOpacity style={styles.headerAvatarBtn}>
              <MaterialIcons name="filter-list" color={colors.secondary} size={26} />
            </TouchableOpacity>
          </View>
        </View>
        ...
        <View style={styles.summaryContainer}>
          ...
          <View style={[styles.mainSummaryCard, { backgroundColor: "#1A1A1A" }]}>
            <View style={styles.cardHeader}>
              <MaterialCommunityIcons name="check-decagram" color={colors.success} size={20} />
      ...
      <ScrollView contentContainerStyle={styles.scrollPad} showsVerticalScrollIndicator={false}>
        ...
        {isLoading ? (
          <View style={{ marginTop: 40 }}><ActivityIndicator size="large" color={colors.secondary} /></View>
        ) : history.length === 0 ? (
          <View style={{ marginTop: 40, alignItems: 'center' }}>
            <MaterialCommunityIcons name="clipboard-text-outline" size={60} color="rgba(255,255,255,0.1)" />
            <Text style={{ color: 'rgba(255,255,255,0.4)', marginTop: 15, fontSize: 16 }}>Aucune mission terminée</Text>
          </View>
        ) : (
                    ...
                    <View style={styles.cardInfo}>
                      <View style={styles.cardHeaderRow}>
                        <View style={styles.timePill}>
                          ...
                          <Text style={styles.timeText}>{new Date(mission.created_at).toLocaleDateString()} {new Date(mission.created_at).toLocaleTimeString().slice(0, 5)}</Text>
                        </View>
                      </View>
                      <Text style={styles.victimName}>{mission.title}</Text>
                      <View style={styles.locationInfo}>
                        <MaterialIcons name="location-on" color={colors.secondary} size={16} />
                        <Text style={styles.addressText} numberOfLines={1}>{mission.location.address}</Text>
                      </View>
                      <View style={styles.cardDivider} />
                      <View style={styles.cardFooterRow}>
                        <View style={[styles.statusBadge, { backgroundColor: specs.color + '15' }]}>
                          <MaterialIcons name={specs.icon} color={specs.color} size={14} />
                          <Text style={[styles.statusBadgeText, { color: specs.color }]}>Terminé</Text>
                        </View>
...
const styles = StyleSheet.create({
  ...
  mainSummaryCard: { flex: 1, height: 110, borderRadius: 32, padding: 20, justifyContent: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.05)" },
  ...
  victimName: { color: "#FFF", fontSize: 18, fontWeight: "900", marginBottom: 8 },
